...
export function TensesReferencePage({navigate}:{navigate:Navigate}){
  ...
  return <main className="reference-page">
    ...
    <section className="tense-reference-section"><ReferenceNav current="tenses" navigate={navigate}/>
      ...
      {mode==='compare'?<div className="comparison-grid">{tenseComparisons.map(comparison=><article className="comparison-card" key={comparison.id}><div className="comparison-visual" aria-hidden="true"><span>{comparison.choices[0]?.name}</span><i>versus</i><span>{comparison.choices[1]?.name}</span></div><p className="eyebrow">Meaning contrast</p><h2>{comparison.title}</h2><p>{comparison.situation}</p><div className="comparison-choices">{comparison.choices.map(choice=><div key={choice.name}><strong>{choice.name}</strong><div className="speakable-example"><p>{choice.example}</p><SpeechButton text={choice.example}/></div><small>{choice.meaning}</small></div>)}</div><RecallCheck prompt={comparison.question} answer={comparison.answer}/></article>)}</div>:<>
