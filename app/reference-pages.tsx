...
import { softwareGuideExamples, softwareTenseComparisonExamples, softwareTenseExamples, softwareVerbExamples, type ExampleContext } from './content/reference/software-examples';
...
const EXAMPLE_CONTEXT_KEY='fluent-path-reference-example-context-v1';
...
function useExampleContext(){
  const [context,setContextState]=useState<ExampleContext>('general');
  useEffect(()=>{const timer=window.setTimeout(()=>{try{if(localStorage.getItem(EXAMPLE_CONTEXT_KEY)==='software')setContextState('software')}catch{/* Ignore unavailable device storage. */}},0);return()=>window.clearTimeout(timer)},[]);
  const setContext=(next:ExampleContext)=>{setContextState(next);try{localStorage.setItem(EXAMPLE_CONTEXT_KEY,next)}catch{/* The choice still works for this visit. */}};
  return {context,setContext};
}

function ExampleContextToggle({context,setContext}:{context:ExampleContext;setContext:(next:ExampleContext)=>void}){
  return <div className="example-context-bar"><div><span className="example-context-icon" aria-hidden="true">{'</>'}</span><p><strong>Example context</strong><small>Switch every example without changing the grammar.</small></p></div><div className="example-context-toggle" role="group" aria-label="Choose example context"><button aria-pressed={context==='general'} className={context==='general'?'active':''} onClick={()=>setContext('general')}>Everyday English</button><button aria-pressed={context==='software'} className={context==='software'?'active software':''} onClick={()=>setContext('software')}>Software engineering</button></div></div>;
}
...
export function TensesReferencePage({navigate}:{navigate:Navigate}){
  ...
  const {context,setContext}=useExampleContext();
  ...
  const examplesFor=(id:string,fallback:string[])=>context==='software'?softwareTenseExamples[id]??fallback:fallback;
  ...
  return <main className="reference-page">
    ...
    <section className="tense-reference-section"><ReferenceNav current="tenses" navigate={navigate}/>
      ...
      <ExampleContextToggle context={context} setContext={setContext}/>
      {mode==='compare'?<div className="comparison-grid">{tenseComparisons.map(comparison=><article className="comparison-card" key={comparison.id}><div className="comparison-visual" aria-hidden="true"><span>{comparison.choices[0]?.name}</span><i>versus</i><span>{comparison.choices[1]?.name}</span></div><p className="eyebrow">Meaning contrast</p><h2>{comparison.title}</h2><p>{comparison.situation}</p><div className="comparison-choices">{comparison.choices.map((choice,index)=>{const example=context==='software'?softwareTenseComparisonExamples[comparison.id]?.[index]??choice.example:choice.example;return <div key={choice.name}><strong>{choice.name}</strong><div className="speakable-example"><p>{example}</p><SpeechButton text={example}/></div><small>{choice.meaning}</small></div>})}</div><RecallCheck prompt={comparison.question} answer={comparison.answer}/></article>)}</div>:<>
...
export function IrregularReferencePage({navigate}:{navigate:Navigate}){
  ...
  const exampleForVerb=(verb:{base:string;example:string})=>context==='software'?softwareVerbExamples[verb.base]??verb.example:verb.example;
  const filtered=irregularVerbs.filter(v=>[...Object.values(v),exampleForVerb(v)].join(' ').toLowerCase().includes(query.toLowerCase())).filter(v=>pattern==='All'||verbPattern(v.base,v.past,v.participle)===pattern).filter(v=>tier==='All'||(tier==='Essential'?essentialVerbs.has(v.base):!essentialVerbs.has(v.base))).filter(v=>mode!=='review'||isReviewItem(review[keyFor('verbs',v.base)]));
...
export function GeneralReferencePage({guideId,navigate}:{guideId:string;navigate:Navigate}){
  ...
  const filtered=guide.items.filter(item=>{const examples=context==='software'?softwareGuideExamples[guide.id]?.[item.id]??item.examples:item.examples;return [item.title,item.pattern,item.explanation,...examples,...item.tags].join(' ').toLowerCase().includes(query.toLowerCase())}).filter(item=>!reviewOnly||isReviewItem(review[keyFor(guide.id,item.id)]));
  return <main className="reference-page"><section className="reference-hero"><p className="eyebrow">{guide.category} · {guide.level}</p><h1>{guide.title},<br/><em>ready to use.</em></h1><p>{guide.description}</p></section><section className="guide-section"><ReferenceNav current={guide.id} navigate={navigate}/><ExampleContextToggle context={context} setContext={setContext}/><div className="library-title"><div><h2>{guide.items.length} key patterns</h2><p>Meaning · Form · Natural examples · Common mistakes · Recall</p></div><label className="search-field"><span>⌕</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder={`Search ${guide.shortTitle.toLowerCase()}…`} aria-label={`Search ${guide.title}`}/></label></div><div className="guide-actions"><button className={reviewOnly?'active':''} onClick={()=>setReviewOnly(!reviewOnly)}>★ {reviewOnly?'Showing review list':'Only my review items'}</button><button onClick={()=>navigate(`/lesson/${guide.lessonId}`)}>Open related lesson →</button></div><div className="guide-grid">{filtered.map((item,index)=><GuideItemCard key={item.id} guideId={guide.id} item={item} index={index} mark={review[keyFor(guide.id,item.id)]} update={update} context={context}/>)}</div>{!filtered.length&&<div className="empty-state"><strong>Nothing here yet.</strong><p>{reviewOnly?'Save an item or mark it Hard or Unsure.':'Try a broader search.'}</p></div>}</section><button className="floating-back" onClick={()=>navigate('/reference')}>← All references</button></main>;
}
