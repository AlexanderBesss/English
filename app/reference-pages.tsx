...
export function TensesReferencePage({navigate}:{navigate:Navigate}){
  ...
  const filtered=tenseReferences.filter(t=>time==='All'||t.time===time).filter(t=>[t.name,t.form,t.use,t.negative,t.question,t.viewpoint,t.contrast,...t.signalWords,...t.examples].join(' ').toLowerCase().includes(query.toLowerCase())).filter(t=>mode!=='review'||isReviewItem(review[keyFor('tenses',t.id)]));
  return <main className="reference-page">
    ...
    <section className="tense-reference-section"><ReferenceNav current="tenses" navigate={navigate}/>
      ...
      {mode==='compare'?<div className="comparison-grid">{tenseComparisons.map(comparison=><article className="comparison-card" key={comparison.id}><p className="eyebrow">Meaning contrast</p><h2>{comparison.title}</h2><p>{comparison.situation}</p><div className="comparison-choices">{comparison.choices.map(choice=><div key={choice.name}><strong>{choice.name}</strong><p>{choice.example}</p><small>{choice.meaning}</small></div>)}</div><RecallCheck prompt={comparison.question} answer={comparison.answer}/></article>)}</div>:<>
...
export function IrregularReferencePage({navigate}:{navigate:Navigate}){
  ...
  const filtered=useMemo(()=>irregularVerbs.filter(v=>Object.values(v).join(' ').toLowerCase().includes(query.toLowerCase())).filter(v=>pattern==='All'||verbPattern(v.base,v.past,v.participle)===pattern).filter(v=>tier==='All'||(tier==='Essential'?essentialVerbs.has(v.base):!essentialVerbs.has(v.base))).filter(v=>mode!=='review'||isReviewItem(review[keyFor('verbs',v.base)])),[query,pattern,tier,mode,review]);
  ...
  return <main className="reference-page"><section className="reference-hero"><p className="eyebrow">Verb reference + retrieval</p><h1>Irregular verbs,<br/><em>learned by pattern.</em></h1><p>Group sound and spelling changes, hide forms, test your memory and keep difficult verbs in a personal review list.</p></section>
    <section className="reference-table-section"><ReferenceNav current="irregular-verbs" navigate={navigate}/><div className="reference-mode-tabs" role="tablist">{(['table','test','review'] as const).map(value=><button role="tab" aria-selected={mode===value} className={mode===value?'active':''} key={value} onClick={()=>{setMode(value);setTestIndex(0)}}>{value==='table'?'Browse & hide forms':value==='test'?'Test yourself':'My review list'}</button>)}</div>
      ...
      {mode==='test'?current?<section className="verb-test-card"><p className="eyebrow">Verb {testIndex%filtered.length+1} of {filtered.length} · Pattern {verbPattern(current.base,current.past,current.participle)}</p><h2>{current.base}</h2><p>Type both missing forms from memory.</p><div className="verb-test-inputs"><label>Past simple<input value={pastInput} disabled={checked} onChange={e=>setPastInput(e.target.value)}/></label><label>Past participle<input value={partInput} disabled={checked} onChange={e=>setPartInput(e.target.value)}/></label></div>{checked&&<div className={`feedback ${correct?'correct':'incorrect'}`} role="status"><strong>{correct?'✓ Correct':'Not quite yet'}</strong><p>{current.base} — {current.past} — {current.participle}</p><p>{current.example}</p></div>}<div className="question-actions">{checked?<button className="button primary" onClick={next}>Next verb →</button>:<button className="button primary" disabled={!pastInput.trim()||!partInput.trim()} onClick={()=>setChecked(true)}>Check forms</button>}</div><ReviewControls id={keyFor('verbs',current.base)} mark={review[keyFor('verbs',current.base)]} update={update}/></section>:<div className="empty-state"><strong>No verbs found.</strong><p>Change the search or filters.</p></div>:<div className="verb-card-list">{filtered.map(v=>{const key=keyFor('verbs',v.base);return <article className="verb-reference-card" key={v.base}><div className="verb-card-heading"><div><span>{verbPattern(v.base,v.past,v.participle)} · {essentialVerbs.has(v.base)?'Essential 20':'Extended'}</span><h2>{v.base}</h2></div><button onClick={()=>speak(`${v.base}, ${v.past}, ${v.participle}. ${v.example}`)} aria-label={`Listen to ${v.base}`}>🔊 Listen</button></div><div className="verb-forms"><p><small>Past simple</small><strong className={hidePast?'covered':''}>{hidePast?'Tap “Hide past” to reveal':v.past}</strong></p><p><small>Past participle</small><strong className={hideParticiple?'covered':''}>{hideParticiple?'Tap “Hide participle” to reveal':v.participle}</strong></p></div><p className="pronunciation">{v.pronunciation}</p><blockquote>{v.example}</blockquote><small>{v.note}</small><ReviewControls id={key} mark={review[key]} update={update}/></article>})}</div>}
...
function GuideItemCard({guideId,item,mark,update}:{guideId:string;item:ReferenceItem;mark?:ReviewMark;update:(id:string,patch:Partial<ReviewMark>)=>void}){
  const [open,setOpen]=useState(false);const key=keyFor(guideId,item.id);
  return <article className="guide-item-card"><div className="guide-item-heading"><div><div className="signal-list">{item.tags.map(tag=><span key={tag}>{tag}</span>)}</div><h2>{item.title}</h2></div><button onClick={()=>setOpen(!open)} aria-expanded={open}>{open?'Collapse':'Study'}</button></div><div className="guide-pattern"><small>Pattern or key distinction</small><strong>{item.pattern}</strong></div><p>{item.explanation}</p><div className="guide-examples">{item.examples.map(example=><blockquote key={example}>{example}</blockquote>)}</div>{open&&<div className="guide-study">{item.mistake&&<div className="micro-mistake"><span>× {item.mistake.wrong}</span><span>✓ {item.mistake.right}</span></div>}<RecallCheck prompt={item.prompt} answer={item.answer}/></div>}<ReviewControls id={key} mark={mark} update={update}/></article>;
}
